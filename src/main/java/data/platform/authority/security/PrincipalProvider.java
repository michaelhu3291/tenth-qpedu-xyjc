...
public class PrincipalProvider implements UserDetailsImpService
{
    /**
     * 通过用户登录帐号查找指定的用户。
     */
	...
	public UserDetails loadUserByUserbua(Map<String, String> userMap) {
		...
		UserDetails result = null ;
	    Map<String, String> entity = getUserService().getUserByBuas( userMap ) ;
         ...
         if( entity != null )
        {
            result = new PrincipalDetails(
               ...
               //entity.getStatus()==1 ? true:false,
                true,
                getUserService().getFunctionAuthoritiesByUser(" ")
                ) ;
        }
        return result ;
	}

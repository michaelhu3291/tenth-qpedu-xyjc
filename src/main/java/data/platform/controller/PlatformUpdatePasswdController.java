...
import data.platform.service.PlatformUserBuaService;
...
public class PlatformUpdatePasswdController extends AbstractBaseController
{
    ...
    public void updateCheckFillingStatus( @RequestParam( "data" ) String data, java.io.PrintWriter out )
    {
        try
        {
        	EntityPlatformUserBua userEntity=userService.load(SecurityContext.getPrincipal().getId());
            ...
            if(!prevPasswd.equals(userEntity.getUser_passwd())){
            ...
            userEntity.setUser_passwd(newPasswd);
            userEntity.setLast_update_time(new Date());
	...
	@Autowired
	private PlatformUserBuaService userService;
}

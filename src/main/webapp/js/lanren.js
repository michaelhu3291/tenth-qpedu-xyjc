...
var jjrmodelidlist=[];  //用于存放从数据库取出的所有节假日的id
	...
	var jjrmodelqclist=[]; //用于存放从数据库取出的所有节假日的全称	
	var jjrmodelendtimelist=[]; //用于存放从数据库取出的所有节假日的结束日期	
	var scheduleList=[]; //用于存放从数据库取出的所有辅导员日程		

$(function(){
	$.ajax({
		type:"POST",
		url:projectName+"/schoolHoliday/schoolHolidaySet.do?command=searchHoliday",
		async:false,
		data:{},
		success:function(json){
				...
				for(var i=0;i<data.length;i++){
					...
					jjrmodeltimelist.push(dateFormat(data[i].START_TIME,"yyyy-MM-dd HH:mm:ss"));
					jjrmodelendtimelist.push(dateFormat(data[i].END_TIME,"yyyy-MM-dd HH:mm:ss"));
					jjrmodelztlist.push(data[i].SHORT_NAME);
					jjrmodelqclist.push(data[i].HOLIDAY_NAME);
				}
			}
		}
	});
...
function getjjrszModelByYear(year){
	/*jjrmodelidlist=['2','2','2'];
	jjrmodeltimelist=['2016-04-14 00:00:00','2016-04-13 00:00:00','2016-04-19 00:00:00']; //这里时间的格式为yyyy-MM-dd HH:mm:ss
	jjrmodelztlist=['2','2','1'];  //1为上班，2为休息 
*/
	var jjrmodelidlist=[],
		jjrmodeltimelist=[], //这里时间的格式为yyyy-MM-dd HH:mm:ss
		jjrmodelendtimelist=[], //这里时间的格式为yyyy-MM-dd HH:mm:ss 结束时间
		jjrmodelztlist=[];  //1为上班，2为休息 
		jjrmodelqclist=[];  //1为上班，2为休息 
		scheduleList=[];  //1为上班，2为休息 
		$.ajax({
			type:"POST",
			url:"../schoolHoliday/schoolHolidaySet.do?command=searchHoliday",
			async:false,
			data:{},
			success:function(json){
					var data=JSON.parse(json);
...
function setRigth(year,yue,day,_this){
	//先清空
	...
	aboluoxssj.appendChild(newElement('br'));
...
function formatByDate(date){
	...
	if(daxx[1]!=""&&daxx[1]!=null){
		if(daxx[2]!=""&&daxx[2]!=null){
	...
	for(var i=0;i<arr.length;i++){
		...
		var datearr=date.split("-");
			if(arr[i].parentNode.className=="aboluo-pervMonthDays"){
				...
				arr[i].setAttribute("onclick","javascript:setRigth("+datearr[0]+","+datearr[1]+","+datearr[2]+",this);javascript:stopBubble(this);");
			}
		for(var n=0;n<jjrmodelidlist.length;n++){
			if(formatByDate(jjrmodeltimelist[n])==formatByDate(date)){
				var div = newElement('span');
				div.setAttribute("class","aboluo-td-type");
				div.setAttribute("title",jjrmodelqclist[n]);
				arr[i].setAttribute("ztid",jjrmodelidlist[n]);
				arr[i].setAttribute("jjrzt",jjrmodelztlist[n]);
				div.innerHTML=jjrmodelztlist[n];
				$(arr[i]).before($(div));
			...
			//如果当前时间在某个节假日的区间则追加假日图标
			if(formatByDate(date)>formatByDate(jjrmodeltimelist[n])&&formatByDate(date)<=formatByDate(jjrmodelendtimelist[n])){
				var div = newElement('span');
				div.setAttribute("class","aboluo-td-type");
				div.setAttribute("title",jjrmodelqclist[n]);
				arr[i].setAttribute("ztid",jjrmodelidlist[n]);

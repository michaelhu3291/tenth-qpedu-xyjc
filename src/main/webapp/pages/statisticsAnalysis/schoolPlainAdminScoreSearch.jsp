...
<script type="text/javascript">
  ...
  //根据登录用户显示其所属的年级和班级
        ...
        listUrl = "../statisticsAnalysis/scoreSearch.do?command=searchScoreForqpAdmin";
        var courseTxt = [];
        var courseVal = [];
        var examName = [];
        var examNumber = [];
        ...
        var getPrint=function(){
        	...
        	if(courseVal !=""){
            	...
            	if(school==null){
            		var printUrl="../statisticsAnalysis/printLook.do?command=searchScoreForqpAdmin&schoolYear="+ schoolYear + "&grade="+grade+"&term="+term+"&examType="+examType+"&examNumberOrStuCode="+examNumberOrStuCode+"&courseVal="+courseVal+"&courseTxt="+courseTxt+"&idx="+idx+"&so="+so+"&examNum="+examNum+"&scoreHtml="+scoreHtml+"&gradeTxt="+gradeTxt;
            	}else{
            		var printUrl="../statisticsAnalysis/printLook.do?command=searchScoreForqpAdmin&schoolYear="+ schoolYear + "&grade="+grade+"&term="+term+"&examType="+examType+"&school="+school+"&examNumberOrStuCode="+examNumberOrStuCode+"&courseVal="+courseVal+"&courseTxt="+courseTxt+"&idx="+idx+"&so="+so+"&examNum="+examNum+"&scoreHtml="+scoreHtml+"&gradeTxt="+gradeTxt;
            	}
			 ...
			 //声明对象
				...
				for (var a = 0; a < courseTxt.length
					...
					_colModel.push(new Object(courseTxt[a],
							courseVal[a], 40, "center",true,true));
				}
	            
				_colModel.push(new Object("总分",
						"Total_Score", 40, "center",true,true));
	...
	var data={
	 ...
	 var url = "../statisticsAnalysis/schoolPlainAdminScoreSearch.do?command=exportExcel&data="+JSON.stringify(data);
